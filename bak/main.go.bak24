...
func initAudio() {
	...
	// AudioContext.resume() returns a Promise. It does not need to be awaited
	// here because this function is called directly from a user keypress.
	if state := audioCtx.Get("state"); state.Type() == js.TypeString && state.String() == "suspended" {
...
func scheduleBeep(freq float64, duration float64, volume float64, delay float64) {
	if !audioInitialized || !enableSounds || audioCtx.IsNull() || audioCtx.IsUndefined() {
		return
	}
	defer func() {
		if r := recover(); r != nil {
	...
	startTime := audioCtx.Get("currentTime").Float() + delay
	osc := audioCtx.Call("createOscillator")
	gain := audioCtx.Call("createGain")
	osc.Set("type", "sine")
	osc.Set("frequency", freq)
	gain.Set("gain", volume)
	osc.Call("connect", gain)
	gain.Call("connect", audioCtx.Get("destination"))
	osc.Call("start", startTime)
	gain.Get("gain").Call("exponentialRampToValueAtTime", 0.001, startTime+duration)
	osc.Call("stop", startTime+duration+0.05)
}

func playBeep(freq float64, duration float64, volume float64) {
	scheduleBeep(freq, duration, volume, 0)
}

func playPaddleHit() {
	playBeep(600, 0.08, 0.2)
}
...
func playMagic() {
	scheduleBeep(1000, 0.06, 0.15, 0)
	scheduleBeep(1400, 0.08, 0.15, 0.1)
}

func playPowerup() {
	scheduleBeep(500, 0.08, 0.15, 0)
	scheduleBeep(700, 0.08, 0.15, 0.1)
	scheduleBeep(900, 0.12, 0.2, 0.2)
}

func playGameOver() {
	scheduleBeep(600, 0.2, 0.2, 0)
	scheduleBeep(400, 0.3, 0.2, 0.25)
}

func playWin() {
	scheduleBeep(500, 0.12, 0.2, 0)
	scheduleBeep(700, 0.12, 0.2, 0.15)
	scheduleBeep(900, 0.2, 0.25, 0.3)
}

func playLevelComplete() {
	scheduleBeep(600, 0.08, 0.2, 0)
	scheduleBeep(800, 0.12, 0.2, 0.12)
}

func playBallLost() {
	scheduleBeep(400, 0.2, 0.15, 0)
	scheduleBeep(300, 0.25, 0.15, 0.2)
}
...
func buildBricksFromLevel(lvl levelData) {
	...
	maxHeight := canvasHeight - 2*margin
	...
	gridOffsetLeft = (canvasWidth - scaledTotalWidth) / 2.0
	...
	if brickOffsetTop >= 0 {
		...
		if offsetTop+scaledTotalHeight > canvasHeight-margin {
			offsetTop = canvasHeight - margin - scaledTotalHeight
		}
	} else {
		scaledTotalHeight := float64(rows)*(scaledHeight+scaledPadding) - scaledPadding
		offsetTop = (canvasHeight - scaledTotalHeight) / 2.0
	}
	...
	for row := 0; row < rows; row++ {
		...
		for col := 0; col < cols; col++ {
			var ch byte = ' '
			if col < len(trimmedLine) {
				ch = trimmedLine[col]
			}
			if ch == ' ' {
				continue
			}
			x := gridOffsetLeft + float64(col)*(scaledWidth+scaledPadding)
			y := offsetTop + float64(row)*(scaledHeight+scaledPadding)
...
func nukeBricks(hitBrick *brick) {
	...
	firstBrick := &bricks[0]
	offsetTop := firstBrick.y

	hitRow := int((hitBrick.y - offsetTop) / (brickHeight + brickPadding))
	...
	for i := range bricks {
		...
		row := int((b.y - offsetTop) / (brickHeight + brickPadding))
...
func updateBall(b *Ball, dt float64, isPrimary bool) {
	...
	if b.y+b.r > canvasHeight {
		// Ball lost ╬ô├ç├┤ handled by caller
		return
	}
...
func update(dt float64) {
	...
	if powerUpTextTimer > 0 {
		powerUpTextTimer -= dt
		if powerUpTextTimer <= 0 && !blackHoleActive && activePowerUp == POWER_NONE {
			powerUpText = ""
		}
	}
	...
	if secondBallActive {
		...
		if primaryLost && !secondLost {
			// Keep playing with the second ball.
			ball = secondBall
			secondBallActive = false
			playBallLost()
		} else if secondLost && !primaryLost {
			// Primary ball is still alive.
			secondBallActive = false
			playBallLost()
	...
	// Check win (all bricks destroyed except unbreakable)
	// Complete the level when no breakable bricks remain.
	allBreakableDead := true

	for _, b := range bricks {
		if b.alive && !b.unbreakable {
			allBreakableDead = false
			break
		}
	}
...
func draw() {
	...
	if secondBallActive {
		drawBall(secondBall.x, secondBall.y, secondBall.angle, palette[7], palette[8])
	}
	...
	ctx.Call("fillText", "Level: "+strconv.Itoa(currentLevelIndex+1)+"/"+strconv.Itoa(len(levels)), 10, 90)

	if powerUpText != "" && powerUpTextTimer > 0 {
		ctx.Set("fillStyle", palette[4])
		ctx.Set("font", "20px sans-serif")
		ctx.Call("fillText", powerUpText, 10, 120)
	}
...
func setupInput() {
	keyDown = js.FuncOf(func(this js.Value, args []js.Value) (ret interface{}) {
		...
		// Initialize audio only on key press (not mouse move)
		if !audioInitialized {
			initAudio()
		}
