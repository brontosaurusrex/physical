...
var (
	...
	lives    int
	gameOver bool
	win      bool // true if all bricks cleared
...
func initBricks() {
	...
	offsetLeft := (canvasWidth - totalWidth) / 2.0

	for row := 0; row < brickRows; row++ {
		for col := 0; col < brickCols; col++ {
			x := offsetLeft + float64(col)*(brickWidth+brickPadding)
...
func draw() {
	...
	if gameOver {
		...
		// Center the text
		width := float64(len(msg)) * 30 // rough estimate
